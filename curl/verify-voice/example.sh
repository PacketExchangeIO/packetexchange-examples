...
#
#   ./verify-voice/example.sh +14155550100 +14155550199 es
set -euo pipefail
...
if [ "$#" -lt 2 ] || [ "$#" -gt 3 ]; then
  echo "Usage: verify-voice/example.sh <to> <caller-id> [language]" >&2
  exit 2
fi
to="$1"
caller_id="$2"
...
started="$(px POST /verify/start "$(jq -nc --arg to "$to" --arg from "$caller_id" --arg lang "$language" \
  '{to: $to, channel: "voice", from: $from, language: $lang}')" \
  -H "X-Idempotency-Key: $(idempotency_key)")"
verification_id="$(jq -r '.data.verificationId' <<<"$started")"
echo "Verification call placed to $to (language: $language)"
