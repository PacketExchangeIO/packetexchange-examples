...
{
    Console.Error.WriteLine("Usage: dotnet run --project verify-voice -- <to> <caller-id> [language]");
    return 2;
}
var to = args[0];
var callerId = args[1];
...
try
{
    var start = (await api.SendAsync<Envelope<VerifyStart>>(
        HttpMethod.Post, "/verify/start", new { to, channel = "voice", from = callerId, language }, idempotent: true)).Data;
    Console.WriteLine($"Verification call placed to {to} (language: {language})");
