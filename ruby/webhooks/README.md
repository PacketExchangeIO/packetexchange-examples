# Receive and verify webhooks (Ruby)
...
From the `ruby` folder (see the [Ruby README](../README.md) for setup; the server needs `bundle install` once, for WEBrick):

```bash
export PACKETEXCHANGE_WEBHOOK_SECRET=...   # shown once when you create the endpoint
bundle exec ruby webhooks/main.rb   # listens on http://localhost:3000/webhooks (PORT to change)
```
