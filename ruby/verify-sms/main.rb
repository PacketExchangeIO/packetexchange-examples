...
def main(args)
  if args.length != 1
    warn "Usage: ruby verify-sms/main.rb <to>"
    exit 2
  end
  to = args[0]

  start = api_request("POST", "/verify/start", body: { to: to, channel: "sms" }, idempotent: true)["data"]
  puts "Verification sent by SMS to #{to}"
