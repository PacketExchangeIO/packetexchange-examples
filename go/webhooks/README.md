# Receive and verify webhooks (Go)
...
From the `go` folder (see the [Go README](../README.md) for setup):

```bash
export PACKETEXCHANGE_WEBHOOK_SECRET=...   # shown once when you create the endpoint
go run ./webhooks   # listens on http://localhost:3000/webhooks (PORT to change)
```
