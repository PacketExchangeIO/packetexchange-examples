...
PacketExchange API examples for Go 1.23, using only the standard library (`net/http`,
`encoding/json`, `crypto/hmac`). No PacketExchange SDK is involved, so each file shows the raw
HTTP API.

## Requirements

- Go 1.23 or later. There are no third-party modules; every example shares the module in
  this folder.
...
```bash
go run ./verify-sms +14155550100
go run ./price-a-number +447700900123
go run ./phone-numbers search 1415
```

Each example prints a usage line when run without arguments. Lint with `go vet ./...`.
