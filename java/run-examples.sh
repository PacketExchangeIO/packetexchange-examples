#!/usr/bin/env bash
# Runs every Java example against the mock API. Use it through scripts/with-mock.sh:
#
#   scripts/with-mock.sh java/run-examples.sh
#
# Fetch the dependency first with: mvn -q dependency:copy-dependencies
set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")"
source ../scripts/lib.sh

if ! ls target/dependency/gson-*.jar >/dev/null 2>&1; then
  echo "Run 'mvn -q dependency:copy-dependencies' in java/ first." >&2
  exit 1
fi

# example <name> <args...>: runs one example with the Java source launcher.
example() {
  local name="$1"
  shift
  java -cp 'target/dependency/*' "$name/Main.java" "$@"
}
...
expect_output 2 "Set PACKETEXCHANGE_API_KEY first" env -u PACKETEXCHANGE_API_KEY java -cp 'target/dependency/*' send-sms/Main.java +14155550100 Riverside
...
PORT="$port" java -cp 'target/dependency/*' webhooks/Main.java &
server_pid=$!
wait_for_port "$port"
